...
app = faust.App('stock-ma-data-aggregator', broker='kafka://10.0.1.138:9092')
    ...
    async for stock_data in stream.group_by(StockData.symbol, StockData.MA_type):
        ...
        if key not in table:
            ...
            table[key] = AggregatedData(
                ...
                current_time=max(datetime.fromisoformat(stock_data.current_time.replace('+08:00', '')),
                                 datetime.fromisoformat(existing.current_time.replace('+08:00', ''))).isoformat() + '+08:00',
                first_data_time=min(datetime.fromisoformat(stock_data.first_data_time.replace('+08:00', '')),
                                    datetime.fromisoformat(existing.first_data_time.replace('+08:00', ''))).isoformat() + '+08:00',
                last_data_time=max(datetime.fromisoformat(stock_data.last_data_time.replace('+08:00', '')),
                                   datetime.fromisoformat(existing.last_data_time.replace('+08:00', ''))).isoformat() + '+08:00',
                sma_value=existing.sma_value + stock_data.sma_value,
                sum_of_vwap=existing.sum_of_vwap + stock_data.sum_of_vwap,
                count_of_vwap=existing.count_of_vwap + stock_data.count_of_vwap,
